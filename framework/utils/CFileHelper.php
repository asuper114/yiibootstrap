...
class CFileHelper {
    ...
    public static function rrmdir($path,$del_dir = true, $level = 0) {

        // Trim the trailing slash
        $path = rtrim($path, DIRECTORY_SEPARATOR); //规范化路径
        
        if (!$current_dir = @opendir($path)) {
            return FALSE;
        }

        while (FALSE !== ($filename = @readdir($current_dir))) {//遍历文件夹中的文件
            if ($filename != "." and $filename != "..") {//readdir打开文件夹后,文件系统默认的返回本级和上级目录的连接符‘.’‘..’所以除了这两个字符 其他的是文件
                if (is_dir($path . DIRECTORY_SEPARATOR . $filename)) {
                    // Ignore empty folders
                    if (substr($filename, 0, 1) != '.') {//排除以点开头的系统隐藏文件夹
                        //如果是个文件夹进行递归
                        self::rrmdir($path . DIRECTORY_SEPARATOR . $filename, $del_dir, $level + 1);
                    }
                } else {
        ...
        @closedir($current_dir); //终止文件夹操作

        if ($del_dir == TRUE AND $level > 0) {//递归删除子文件夹里的文件后再将文件夹删除掉
            return @rmdir($path);
        }
